...
class DrawablesRendererImpl(var context: Activity, var renderer: Renderer): DrawablesRenderer {
    private var drawablesContent: JSONArray? = null
    override fun load(drawables: JSONArray) {
        this.drawablesContent = drawables
    }

    override fun getById(did: String): Drawable? {
    ...
    private fun setLayerOptions(builder: LayerDrawableBuilder, layer: JSONObject) {
        layer.sGetJsonObject(F.options){ options ->
            ...
            options.sGetString(F.gravity){
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    builder.gravity(renderer.factory.gravityParser.parse(it))
                }
            }
        }
    }
    ...
    private fun setDrawableCornerRadii(builder: DrawableBuilder, definition: JSONObject) {
        if(definition.has(F.cornerRadii))
        {
            ...
            if(cornerRadii.contains(" "))
            {
                ...
                if(parts.size==4)
                {
                    var topLeft = renderer.factory.dimensionParser.parse(parts[0],renderer)
                    var topRight = renderer.factory.dimensionParser.parse(parts[1],renderer)
                    var bottomRight = renderer.factory.dimensionParser.parse(parts[2],renderer)
                    var bottomLeft = renderer.factory.dimensionParser.parse(parts[3],renderer)
    ...
    private fun setDrawableRadius(builder: DrawableBuilder, definition: JSONObject) {
        if(definition.has(F.cornerRadius))
        {
            ...
            var num = renderer.factory.dimensionParser.parse(cornerRadius,renderer)
